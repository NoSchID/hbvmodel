...
imperial_model <- function(timestep, pop, parameters, sim_starttime) {
  ...
  with(as.list(parameters), {
    ...
    # No new infections in those treated, but could add incident recoveries
    ...
    dcum_infected_births <- sum(fertility_rate *
                                  (rowSums((1-bdvacc_cov) * mtct_prob_e * pop[index$ages_wocba,HBeAg_pos,1]) +
                                     ...
                                     rowSums((1-bdvacc_cov) * mtct_prob_treat * pop[index$ages_wocba,Treated_carriers,1])+
    ...
    res <- c(dpop, dcum_deaths, dcum_infections, dcum_chronic_infections,
             ...
             dcum_screened_infections, dcum_screened_chronic_infections,
             dcum_screened_hbv_deaths, dcum_treated_hbv_deaths)
...
parameter_list <- list(
  ...
  bdvacc_cov = 0.8,                          # birth dose vaccine coverage
...
# Load functions for output plots ----

# Load functions to extract outcomes of interest
