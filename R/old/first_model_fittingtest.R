...
hbv_model <- function(times, pop, parameters, b1){
...
run_model <- function(init_pop, times, parameters, b1, b2, b3) {
...
##  toReturn <- list(data, as.numeric(pop_by_age[2000,]), as.numeric(prev_by_age[2000,]))
...
likelihood_function <- function(b1) {
  temp <- run_model(init_pop, times, parameters, b1 = b1)
  LL <- temp["LL"]
  return(LL) }

inits_test <- c(b1 = 0)
optim(fn=likelihood_function, par = inits_test, control = list(fnscale=-1))
optimize(likelihood_function, interval = c(0,1), maximum = TRUE)

#likelihood_inputs <- run_model(init_pop, times, parameters)
#explore_likelihood <- cbind(observed = likelihood_inputs[[1]], n = likelihood_inputs[[2]], predicted = likelihood_inputs[[3]])
#dbinom(x=round(explore_likelihood[,"observed"]), size =round(explore_likelihood[,"n"]), prob = explore_likelihood[,"predicted"], log = TRUE)
...
parameters <- c(gamma_acute = 8, sag_loss = sag_loss,   # 0.01
                ...
                mtct_prob_a = 0.711, mtct_prob_i = 0.109,
                p_chronic = p_chronic)
